...
### 2.4 webdav_config (WebDAV配置表)

| 字段名 | 类型 | 长度 | 允许NULL | 默认值 | 说明 |
|--------|------|------|---------|--------|------|
| id | BIGINT | - | NO | AUTO_INCREMENT | 主键ID |
| user_id | BIGINT | - | NO | - | 所属用户ID |
| server_url | VARCHAR | 500 | NO | - | WebDAV服务器地址 |
| username | VARCHAR | 255 | NO | - | 用户名 |
| password | VARCHAR | 500 | NO | - | 加密密码 (AES-256) |
| last_sync_at | DATETIME | - | YES | NULL | 最后同步时间 |
| sync_interval | INT | - | NO | 30 | 同步间隔 (分钟) |
| created_at | DATETIME | - | NO | CURRENT_TIMESTAMP | 创建时间 |
| updated_at | DATETIME | - | NO | CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP | 更新时间 |
...
### 2.5 sync_history (同步历史表)

| 字段名 | 类型 | 长度 | 允许NULL | 默认值 | 说明 |
|--------|------|------|---------|--------|------|
| id | BIGINT | - | NO | AUTO_INCREMENT | 主键ID |
| user_id | BIGINT | - | NO | - | 所属用户ID |
| sync_id | VARCHAR | 100 | NO | - | 同步ID |
| status | VARCHAR | 20 | NO | - | 状态: pending/in_progress/completed/failed |
| started_at | DATETIME | - | NO | - | 开始时间 |
| completed_at | DATETIME | - | YES | NULL | 完成时间 |
| uploaded | INT | - | NO | 0 | 上传数量 |
| downloaded | INT | - | NO | 0 | 下载数量 |
| conflicts | INT | - | NO | 0 | 冲突数量 |
